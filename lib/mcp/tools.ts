    ...
    scoped("tools_documentation", (_args) =>
      withToolLogging("tools_documentation", undefined, () => {
        const text = [
          ...
          "A successful dry run on execute_transfer or execute_contract_call arrives as data, the tool result content:",
          '- {success: true, status: "simulated", from, to, value, gasEstimate, simulatedReturnValue, wouldRevert: false}',
          "A failed dry run does NOT arrive as data. The underlying API returns HTTP 400 (deterministic validation/revert) or 503 (simulator/RPC unavailable), and any non-2xx makes the tool call fail: isError: true, with a text message, not a parseable object -- there is no success or wouldRevert field to branch on. Always branch on isError, never on parsing the failure text as JSON:",
          "- A revert, or a 400 with a machine-readable code (currently only insufficient_balance), gets its text enriched with an appended Reason / Reason code / Next step block after the original 'API call failed: 400 ... - {...}' line",
          "- An uncoded validation 400, and every 503 (simulator itself failed -- RPC/infra, not a signal the call would succeed), get NO enrichment: the bare 'API call failed: <status> ... - {...}' string, with the failure JSON surviving only as a fragment of that error text",
          ...
          "- Condition met, action is a write: follows the execute_transfer/execute_contract_call rule above -- a successful dry run arrives as data (the full simulate shape plus executed and conditionResult); a failed one is isError text, not data",
