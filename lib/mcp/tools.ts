    ...
    scoped("tools_documentation", (_args) =>
      withToolLogging("tools_documentation", undefined, () => {
        const text = [
          ...
          "A failed dry run does NOT arrive as data. The underlying API returns a non-2xx status -- including 400 (deterministic validation/revert), 422 (WALLET_NOT_CONFIGURED -- no wallet set up for the org, checked before simulation and often the first failure on a fresh org; fix it once, do not retry), 503 (simulator/RPC unavailable), 403 (scope check failed), and 429 (rate limited) -- and any non-2xx makes the tool call fail: isError: true, with a text message, not a parseable object -- there is no success or wouldRevert field to branch on. Always branch on isError, never on parsing the failure text as JSON:",
