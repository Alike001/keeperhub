    ...
    scoped("tools_documentation", (_args) =>
      withToolLogging("tools_documentation", undefined, () => {
        const text = [
          ...
          "A successful dry run on execute_transfer or execute_contract_call arrives as data, the tool result content. Exception: a view/pure function called through execute_contract_call with simulate: true never reaches the simulator at all -- it dispatches straight to the normal read path and returns {result: ...} at HTTP 200, with no success, status, or wouldRevert field:",
          '- {success: true, status: "simulated", from, to, value, gasEstimate, simulatedReturnValue, wouldRevert: false}',
          "A failed dry run does NOT arrive as data. The underlying API returns HTTP 400 (deterministic validation/revert), 422 (WALLET_NOT_CONFIGURED -- no wallet set up for the org, checked before simulation and often the first failure on a fresh org; fix it once, do not retry), or 503 (simulator/RPC unavailable), and any non-2xx makes the tool call fail: isError: true, with a text message, not a parseable object -- there is no success or wouldRevert field to branch on. Always branch on isError, never on parsing the failure text as JSON:",
          ...
          '- For reference, the failure JSON embedded in that text has shape {success: false, status: "simulated", from, to, value, error, failureKind, wouldRevert, revertReason?, code?, balanceWei?, requiredWei?, shortfallWei?, nativeSymbol?, originalError?, undecodedRevertData?}; failureKind is "validation", "revert", or "unavailable"',
          ...
          "- Condition met, action is a write: follows the execute_transfer/execute_contract_call rule above -- a successful dry run arrives as data (the full simulate shape plus executed and conditionResult); a failed one is isError text embedding the same failure JSON plus executed and conditionResult, not data",
