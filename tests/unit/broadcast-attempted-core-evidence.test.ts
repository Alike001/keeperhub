...
describe.each(cores)("%s broadcastAttempted evidence", (path) => {
  ...
  it("uses the structural pre-broadcast tag instead of error text", () => {
    ...
    expect(source).toMatch(/broadcastAttempted:[\s\S]{0,240}isPreBroadcastNetworkError\(error\)/);
  });
  it("fails closed when no structural tag is present", () => {
    expect(source).not.toMatch(/broadcastAttempted:[\s\S]{0,240}(?:ECONNREFUSED|connection refused)/i);
  });
});
