...
Base and Arbitrum only. Both reads return wei with 18 decimals, the same as their Ethereum counterparts on the savings vault. Use these slugs when the workflow runs on Base or Arbitrum; workflows already saved against Vault Share Balance or Vault Total Supply keep reading the bridged token on those chains.
