...
The picker shows each service's **id** next to its name, in grey monospace, and so does the field once one is chosen. That is deliberate: the node stores the id and reads the name from PagerDuty on every load, so a service renamed there quietly appears under its new name. The id is the part that does not move. If the account cannot be read at all, the field shows the stored id rather than falling back to "Select a service", so a configured node never reads as an empty one.
